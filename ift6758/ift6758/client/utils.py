...
def add_milestone2_advanced_metrics(events_df):
    ...
    for i in range(events_df.shape[0]):
        ...
        Home_minor_exp_end = Home_minor_exp_end[
            Home_minor_exp_end > events_df.at[i, "game_sec"]
        ]
        Home_DM_exp_end = Home_DM_exp_end[Home_DM_exp_end > events_df.at[i, "game_sec"]]
        Home_major_exp_end = Home_major_exp_end[
            Home_major_exp_end > events_df.at[i, "game_sec"]
        ]

        Guest_minor_exp_end = Guest_minor_exp_end[
            Guest_minor_exp_end > events_df.at[i, "game_sec"]
        ]
        Guest_DM_exp_end = Guest_DM_exp_end[
            Guest_DM_exp_end > events_df.at[i, "game_sec"]
        ]
        Guest_major_exp_end = Guest_major_exp_end[
            Guest_major_exp_end > events_df.at[i, "game_sec"]
        ]

        # Number of friendly non-goalie skaters right before the event
        if (
            Home_minor_exp_end.size + Home_DM_exp_end.size + Home_major_exp_end.size
        ) > 2:
            PreEvent_Home_Skaters = 3
        else:
            PreEvent_Home_Skaters = 5 - (
                Home_minor_exp_end.size + Home_DM_exp_end.size + Home_major_exp_end.size
            )

        # 10. Number of opposing non-goalie skaters
        if (
            Guest_minor_exp_end.size + Guest_DM_exp_end.size + Guest_major_exp_end.size
        ) > 2:
            PreEvent_Guest_Skaters = 3
        else:
            PreEvent_Guest_Skaters = 5 - (
                Guest_minor_exp_end.size
                + Guest_DM_exp_end.size
                + Guest_major_exp_end.size
            )
        ...
        PreEvent_PowerPlayTimerHome = (
            events_df.at[i, "game_sec"] - Home_power_startstamp
        )
        PreEvent_PowerPlayTimerGuest = (
            events_df.at[i, "game_sec"] - Guest_power_startstamp
        )
        ...
        if (events_df.at[i, "type"] == "PENALTY") and (
            events_df.at[i, "shooter_team_name"] == events_df.at[i, "away_team"]
        ):

            # Penalty is a doulbe minor penalty
            if events_df.at[i, "penaltyMinutes"] == 4:
                # Expected ending time of the double minor penalty
                Guest_DM_exp_end = np.append(
                    Guest_DM_exp_end, (events_df.at[i, "game_sec"] + (60 * 4))
                )

            # Penalty is a major penalty
            elif events_df.at[i, "penaltyMinutes"] == 5:
                # Expected ending time of the major penalty
                Guest_major_exp_end = np.append(
                    Guest_major_exp_end, (events_df.at[i, "game_sec"] + (60 * 5))
                )

            # Penalty is a minor penalty
            elif events_df.at[i, "penaltyMinutes"] == 2:
                # Expected ending time of the minor penalty
                Guest_minor_exp_end = np.append(
                    Guest_minor_exp_end, (events_df.at[i, "game_sec"] + (60 * 2))
                )

        # Home team got a penalty, guest team powerplay
        elif (events_df.at[i, "type"] == "PENALTY") and (
            events_df.at[i, "shooter_team_name"] == events_df.at[i, "home_team"]
        ):

            # Penalty is a doulbe minor penalty
            if events_df.at[i, "penaltyMinutes"] == 4:
                # Expected ending time of the double minor penalty
                Home_DM_exp_end = np.append(
                    Home_DM_exp_end, (events_df.at[i, "game_sec"] + (60 * 4))
                )

            # Penalty is a major penalty
            elif events_df.at[i, "penaltyMinutes"] == 5:
                # Expected ending time of the major penalty
                Home_major_exp_end = np.append(
                    Home_major_exp_end, (events_df.at[i, "game_sec"] + (60 * 5))
                )

            # Penalty is a minor penalty
            elif events_df.at[i, "penaltyMinutes"] == 2:
                # Expected ending time of the minor penalty
                Home_minor_exp_end = np.append(
                    Home_minor_exp_end, (events_df.at[i, "game_sec"] + (60 * 2))
                )

        # Goal occured, if guest team scored, clear home team penalty
        elif (events_df.at[i, "type"] == "GOAL") and (
            events_df.at[i, "shooter_team_name"] == events_df.at[i, "away_team"]
        ):
...
def model_preprocess(data):
    ...
    X_data_sf = X_data[selected_feats]
    X_data_sf = X_data_sf.dropna(axis=0)

    for col in MODEL_COLS:
        if col not in X_data_sf:
            X_data_sf[col] = 0

    test_data = X_data_sf[MODEL_COLS]

    return test_data


def process_data(data, game_id: int = 2021020329, data_dir: str = "./data/"):
    print(type(data))
    game_cleaned = parse_game_data(game_id, game_data=data)

    test_data = model_preprocess(game_cleaned)
    test_data.to_csv(os.path.join(data_dir, f"{game_id}.csv"))
